CREATE TABLE sistema_esgoto.valvulas (
    ...
    tipo smallint REFERENCES sistema_esgoto.tipo_valvula (id) NOT NULL,
    funcao smallint REFERENCES sistema_esgoto.tipo_valvula_funcao (id) NOT NULL,
    diametro smallint CHECK (diametro BETWEEN 0 AND 1000) NOT NULL,
    acionamento smallint REFERENCES sistema_esgoto.tipo_valvula_acionamento (id),
    acesso smallint REFERENCES sistema_esgoto.tipo_valvula_acesso (id),
    profundidade numeric(3, 2) CHECK (profundidade BETWEEN 0 AND 10),
    posicao smallint REFERENCES sistema_esgoto.tipo_valvula_posicao (id) NOT NULL,
