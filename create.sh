...
# create functions
find $(dirname $0)/data_definition -type f -name "function_*" \
    -exec psql -h 'localhost' -p '5432' -d 'modelo_dados_saneamento' -U 'postgres' -f {} \;

# create types/tables
find $(dirname $0)/data_definition -type f -name "type_*" \
    -exec psql -h 'localhost' -p '5432' -d 'modelo_dados_saneamento' -U 'postgres' -f {} \;

# create tables
find $(dirname $0)/data_definition -type f -name "table_*" -print0 | sort -z | xargs -0 -I{} \
    psql -h 'localhost' -p '5432' -d 'modelo_dados_saneamento' -U 'postgres' -f "{}" 
